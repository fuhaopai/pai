function add() {
	$.ligerDialog.open({
		height:600,
		width: 800,
		title : '新增pai_message_record ',
		url: __ctxPath + '/admin/pai/message/messageRecord/edit.do', 
		showMax: true,
		showToggle: true,
		showMin: true,
		isResize: true,
...
function editDialog(id) {
	$.ligerDialog.open({
		height:600,
		width: 800,
		title : '修改pai_message_record ',
		url: __ctxPath + '/admin/pai/message/messageRecord/edit.do?id=' + id, 
		showMax: true,
		showToggle: true,
		showMin: true,
		isResize: true,
...
function deleteRow()
{
	...
	if(selected){
		$.ligerDialog.confirm('请确认是否删除该记录', function (yes)
				{
	            	if(yes){
	            		var url = __ctxPath + "/admin/pai/message/messageRecord/delete.do";
