function add() {
	$.ligerDialog.open({
		...
		width: 800,
		title : '新增角色-用户 ',
		url: __ctxPath + '/admin/pai/auth/authRoleUser/edit.do', 
		showMax: true,
		showToggle: true,
		showMin: true,
		isResize: true,
...
function editDialog(id) {
	$.ligerDialog.open({
		...
		width: 800,
		title : '修改角色-用户 ',
		url: __ctxPath + '/admin/pai/auth/authRoleUser/edit.do?id=' + id, 
		showMax: true,
		showToggle: true,
		showMin: true,
		isResize: true,
...
function deleteRow()
{
	...
	if(selected){
		$.ligerDialog.confirm('请确认是否删除该记录', function (yes)
				{
	            	if(yes){
	            		var url = __ctxPath + "/admin/pai/auth/authRoleUser/delete.do";
